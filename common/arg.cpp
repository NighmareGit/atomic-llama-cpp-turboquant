...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--override-moe-top-k"}, "N",
        "override the number of routed experts (MoE top-k) executed during inference (0 = native, e.g. 1 forces top-1 routing on native top-2 models) [EXPERIMENTAL]",
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value: must be >= 0");
            }
            params.override_moe_top_k = value;
        }
    ).set_env("LLAMA_ARG_OVERRIDE_MOE_TOP_K").set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY, LLAMA_EXAMPLE_BENCH}));
