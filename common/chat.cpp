...
static common_chat_params common_chat_params_init_laguna(const common_chat_template &    tmpl,
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) {
        ...
        // In this poolside/Laguna template the generation_prompt is "<assistant>"
        // with NO trailing newline — the think tag follows directly: "<assistant><think>"
        // (thinking enabled) or "<assistant></think>" (disabled). Consume it, then
        // capture reasoning. The model's output therefore starts with the reasoning
        // text (no leading <think>). NOTE: the \n here was a hardcoded bug — it made
        // the PEG head literal "<assistant>\n</think>" never match the prepended input.
        const std::string GEN_PROMPT = "<assistant>";
        auto head = p.eps();
        if (extract_reasoning && inputs.enable_thinking) {
