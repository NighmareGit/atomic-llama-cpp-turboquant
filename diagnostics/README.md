...
| [pipeline-plus-dual-gpu-garble/](pipeline-plus-dual-gpu-garble/) | KV-cache garbling on multi-GPU with GGML_PIPELINE_PLUS=1 (target + draft contexts) | OPEN | workaround: `GGML_PIPELINE_PLUS=0`. Fix locally (romulus) first, then triton, then 4-GPU cluster |

## Agent tickets

| Ticket | Title | Status |
|--------|-------|--------|
| [pipeline-plus-dual-gpu-garble/ISSUE.md](pipeline-plus-dual-gpu-garble/ISSUE.md) | Fix multi-GPU target-context KV garble under GGML_PIPELINE_PLUS (debug+fix loop) | open / ready-for-agent |
