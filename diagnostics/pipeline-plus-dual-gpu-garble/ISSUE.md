...
|------|-----|----------|----------------------------|-------------------|
| **triton** (canonical repro) | `192.168.8.23` | `hunter` | <redacted> (`B6_TRITON_PASS` / `PATHB_TRITON_SSH_PASS`) | Dual native CUDA 3090+3070; report was run here |
...
export PATHB_TRITON_SSH=user@192.168.8.23
export PATHB_TRITON_SSH_PASS=<redacted>   # or B6_TRITON_PASS=<redacted>
# Prefer: ssh-copy-id once, then drop password env

ssh user@192.168.8.23
# password default in scripts: <redacted>
```
...
```bash
ssh user@192.168.8.23   # pass default in scripts: <redacted>
