...
```bash
# Prefer: ssh user@192.168.8.108  (key auth; no password in scripts)
...
Triton is Ubuntu 24.04 at `user@192.168.8.23`. Repo path may be `~/projects/atomic-llama-cpp-turboquant` (single-nested; verify with `git rev-parse --show-toplevel`).
...
rsync -avz user@192.168.8.108:~/atomic-llama-cpp-turboquant/benches/path-b-plus/b6-2gpu-f-triton-n384-no-partial/ \
...
|------|----------|------------|
| SSH passwords in scripts (<redacted>) | Medium | Use SSH keys; tokens in `~/tokens/` mode 600 |
