...
| B+13b | Issue split-2 deferred GETs **before** `wait_copy_slot` when dst is local gather | Hide slot wait behind wire transfer |
| B+13c | B+15: prefetch at split 1 **start** (already partial) + avoid double `event_synchronize` at split 1 end | More overlap window |
