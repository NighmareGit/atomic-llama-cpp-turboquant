...
5. **D6.10 GPU Event Pipelining Fix** — 🔴 NEXT. `input_copy_slow` is GPU sync wait (~3,500 µs per SLOW step). With overlapping iterations (Split 2 GPU of iteration N + Split 0/1 of N+1), wait drops to ~2,158 µs. Savings: ~1,300 µs/SLOW step (~10% of 12,946 µs). Requires per-backend GPU events (currently NULL on CPU gather backend) + n_copies>1 overlap. See `docs/tickets/path-d-tickets.md` D6.10.
