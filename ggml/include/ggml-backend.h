...
extern "C" {
    ...
    */
    ...
    // Get per-backend compute timing from the last scheduler run (microseconds)
    GGML_API int64_t              ggml_backend_sched_get_backend_timing_us(ggml_backend_sched_t sched, int backend_id);
...
#define GGML_SCHED_MAX_STAGES 8
    ...
    // D6: Multi-seq (Mode B) per-sequence event API.
    // Each concurrent sequence owns an independent event array so events
    // never alias across sequences regardless of concurrency level.
    GGML_API void ggml_sched_gpipe_init_multi(ggml_backend_sched_t sched, int n_stages, int n_seqs);
...
#define GGML_BACKEND_META_MAX_DEVICES 16
    ...
    GGML_API void ggml_backend_release_cached_memory(ggml_backend_t backend);
