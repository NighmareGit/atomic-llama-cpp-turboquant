...
// B+8 F2: recompute which backends must quiesce copy slot at pipeline_barrier.
static void ggml_backend_sched_update_barrier_src_mask(ggml_backend_sched_t sched) {
    sched->barrier_copy_src_mask = 0;
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            if (input_backend_id >= 0 && input_backend_id != split_backend_id) {
                sched->barrier_copy_src_mask |= (1u << input_backend_id);
            }
        }
    }
}
...
static void ggml_backend_sched_wait_producer(
    ...
    if (ggml_backend_is_rpc(producer_backend)) {
        ...
        if (defer_rpc_pending) {
            if (input_cpy != nullptr && ggml_backend_rpc_download_pending_for_dst(input_cpy)) {
                return;
            }
            if (input_cpy != nullptr && ggml_backend_rpc_relay_pending_for_dst(input_cpy)) {
                return;
            }
        }
    }
...
// Same-host multi-endpoint relay: wait producer then sync GET+SET before graph_compute.
static void ggml_backend_sched_flush_rpc_relays_for_split(
        ggml_backend_sched_t sched, int split_id, int split_backend_id, ggml_backend_t split_backend,
        struct ggml_backend_sched_split * split) {
    const ggml_tensor * relay_dsts[GGML_SCHED_MAX_SPLIT_INPUTS];
    int n_relay_dsts = 0;
    ...
    for (int input_id = 0; input_id < split->n_inputs; input_id++) {
        ...
        if (!ggml_backend_rpc_relay_pending_for_dst(input_cpy)) {
            continue;
        }
        ggml_backend_t producer_backend = ggml_backend_sched_get_tensor_backend(sched, input);
        const int producer_bid = ggml_backend_sched_backend_id(sched, producer_backend);
        ggml_backend_sched_wait_producer(
            sched, split_id, split_backend_id, split_backend, producer_backend, producer_bid, input_cpy, false);
        relay_dsts[n_relay_dsts++] = input_cpy;
    }
    ...
    ggml_backend_rpc_flush_pending_relays_for_dst(relay_dsts, (size_t) n_relay_dsts);
    const auto flush_us = std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::steady_clock::now() - flush_t0).count();
    if (sched_trace_lvl() && flush_us > 0) {
        sched_trace_emit(split_id, split_backend_id, sched->cur_copy, "rpc_relay_flush", flush_us);
    }
}
...
static int ggml_backend_sched_prefetch_gather_rpc_inputs(
        ...
        struct ggml_backend_sched_split * gather_split,
        int gather_bid,
        ggml_backend_t gather_backend) {
    int n_prefetch = 0;

    for (int i = 0; i < gather_split->n_inputs; i++) {
        struct ggml_tensor * inp = gather_split->inputs[i];
        if (inp->flags & GGML_TENSOR_FLAG_INPUT) {
            continue;
        }
        struct ggml_tensor * inp_cpy = tensor_copy(inp, gather_bid, sched->cur_copy);
        ...
        if (buf_src != nullptr && buf_dst != nullptr &&
            ...
            if (ggml_backend_rpc_try_download_tensor(gather_backend, inp, inp_cpy)) {
                n_prefetch++;
            }
        }
    }
    if (gather_split->graph.n_nodes > 0) {
        struct ggml_tensor * node0 = gather_split->graph.nodes[0];
        if (node0->op == GGML_OP_MUL_MAT_ID && node0->src[2] != nullptr) {
            struct ggml_tensor * ids = node0->src[2];
            ggml_backend_buffer_t ids_buf = ids->view_src ? ids->view_src->buffer : ids->buffer;
            if (ids_buf != nullptr && ggml_backend_buffer_is_rpc(ids_buf)) {
                struct ggml_tensor * ids_cpy = tensor_copy(ids, gather_bid, sched->cur_copy);
                if (ggml_backend_rpc_try_download_tensor(gather_backend, ids, ids_cpy)) {
                    n_prefetch++;
                }
            }
        }
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        if (ggml_backend_is_rpc(split_backend) && split_id + 1 < sched->n_splits) {
            ...
            if (next_backend != nullptr && !ggml_backend_is_rpc(next_backend)) {
                ...
                const auto pf_us = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::steady_clock::now() - pf_t0).count();
                if (sched_trace_lvl() && n_pf > 0) {
                    sched_trace_emit(split_id, split_backend_id, sched->cur_copy, "rpc_prefetch_start", pf_us);
                }
            }
        }
        ...
        if (!ggml_backend_is_rpc(split_backend)) {
            ...
            for (int i = 0; i < split->n_inputs; i++) {
                ...
                struct ggml_tensor * inp_cpy = tensor_copy(inp, split_backend_id, sched->cur_copy);
                ggml_backend_buffer_t buf_src = inp->view_src ? inp->view_src->buffer : inp->buffer;
                if (buf_src != nullptr && ggml_backend_buffer_is_rpc(buf_src) &&
                    !ggml_backend_buffer_is_rpc(inp_cpy->buffer) &&
                    ggml_backend_rpc_download_pending_for_dst(inp_cpy)) {
                    gather_flush_dsts[n_gather_flush++] = inp_cpy;
                }
            }
            if (split->graph.n_nodes > 0) {
                struct ggml_tensor * node0 = split->graph.nodes[0];
                if (node0->op == GGML_OP_MUL_MAT_ID && node0->src[2] != nullptr) {
                    ...
                    ggml_backend_buffer_t ids_buf = ids->view_src ? ids->view_src->buffer : ids->buffer;
                    if (ids_buf != nullptr && ggml_backend_buffer_is_rpc(ids_buf)) {
                        struct ggml_tensor * ids_cpy = tensor_copy(ids, split_backend_id, sched->cur_copy);
                        if (ggml_backend_rpc_download_pending_for_dst(ids_cpy)) {
                            gather_flush_dsts[n_gather_flush++] = ids_cpy;
                        }
                    }
                }
            }
            if (n_gather_flush > 0) {
                const auto flush_t0 = std::chrono::steady_clock::now();
                ggml_backend_rpc_flush_pending_downloads_for_dst(gather_flush_dsts, (size_t) n_gather_flush);
                const auto flush_us = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::steady_clock::now() - flush_t0).count();
                if (flush_us > 0) {
                    sched_trace_emit(split_id, split_backend_id, sched->cur_copy, "rpc_gather_flush", flush_us);
                }
            }
        }
        ...
        for (int input_pass = 0; input_pass < 2; input_pass++) {
            ...
            }
            {
                    ...
                    if (prod_buf != nullptr && ggml_backend_buffer_is_rpc(prod_buf)) {
                        ...
                        if (!ggml_backend_buffer_is_rpc(input_cpy->buffer) ||
                            ggml_backend_rpc_relay_pending_for_dst(input_cpy)) {
                            ggml_backend_sched_wait_producer(
                                sched, split_id, split_backend_id, split_backend, input_backend, producer_bid, input_cpy, true);
                        }
                    }
                }
                ...
                if (split->graph.n_nodes > 0 &&
                    ...
                    // B+10/B+14: stream wait on copy-slot event (non-blocking host on GPU backends).
                    {
                    ...
                    {
                        ggml_backend_buffer_t ids_buf = ids_tensor->view_src ? ids_tensor->view_src->buffer : ids_tensor->buffer;
                        if (ids_buf != nullptr && ggml_backend_buffer_is_rpc(ids_buf)) {
                            struct ggml_tensor * ids_cpy = tensor_copy(node->src[2], split_backend_id, sched->cur_copy);
                            if (ggml_backend_rpc_download_pending_for_dst(ids_cpy)) {
                                const ggml_tensor * flush_dst = ids_cpy;
                                ggml_backend_rpc_flush_pending_downloads_for_dst(&flush_dst, 1);
                            }
                            ids_tensor = ids_cpy;
                    ...
                    if (ids_tensor != prev_ids_tensor) {
                        ...
                        // B+14: blocking get avoids event_synchronize on the full backend stream (~2ms on CUDA gather).
...
// D6.8: per-sequence event arrays. Each concurrent sequence owns an independent
// event array so events never alias across sequences regardless of concurrency.
// n_seqs controls how many event arrays to allocate (1 = single-seq, N = N concurrent seqs).
void ggml_sched_gpipe_init_multi(ggml_backend_sched_t sched, int n_stages, int n_seqs) {
    GGML_ASSERT(sched);
    GGML_ASSERT(n_seqs <= GGML_SCHED_MAX_BACKENDS);

    if (n_stages <= 0 || n_seqs <= 0) {
        sched->n_gpipe_stages = 0;
        sched->n_gpipe_seqs = 0;
        return;
    }
...
// D6.8: wait on a GPipe stage event for a specific sequence.
// split_id < 0 drains all stages for the given sequence (pipeline flush).
void ggml_sched_gpipe_wait_seq(ggml_backend_sched_t sched, int split_id, int seq_id) {
    GGML_ASSERT(sched);
    if (sched->n_gpipe_stages == 0 || seq_id < 0 || seq_id >= sched->n_gpipe_seqs) {
        return;
    }
...
void ggml_sched_gpipe_wait(ggml_backend_sched_t sched, int split_id) {
    ggml_sched_gpipe_wait_seq(sched, split_id, 0);
}

// D6.8: record a stage-complete event on the gather backend for the given sequence.
void ggml_sched_gpipe_record_seq(ggml_backend_sched_t sched, int stage_id, int seq_id) {
    GGML_ASSERT(sched);
...
void ggml_backend_sched_free(ggml_backend_sched_t sched) {
    ...
    for (int seq = 0; seq < sched->n_gpipe_seqs; seq++) {
        for (int s = 0; s < sched->n_gpipe_stages; s++) {
            ggml_backend_event_free(sched->gpipe_events[seq * GGML_SCHED_MAX_STAGES + s]);
        }
    }
    free(sched->gpipe_events);
...
bool ggml_backend_sched_alloc_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    sched->is_alloc = true;
    ggml_backend_sched_update_barrier_src_mask(sched);
