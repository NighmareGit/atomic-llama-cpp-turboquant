...
| **triton worker** | `hunter@192.168.8.23` → `~/projects/atomic-llama-cpp-turboquant` | Ubuntu **24.04** (was Windows in old docs) |
| **JUPITER** | `192.168.8.21:50053` | ping OK; **TCP connect times out** from remus — blocked |

**SSH (triton):** `hunter@192.168.8.23`, password `12345` (use `pexpect` on remus — no `sshpass`).
...
2. Installed build deps: `cmake`, `ninja-build`, `libopenblas-dev`, `libomp-dev` (`sudo apt`, pw `12345`)
