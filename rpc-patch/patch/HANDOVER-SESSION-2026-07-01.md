...
| **triton worker** | `user@192.168.8.23` → `~/projects/atomic-llama-cpp-turboquant` | Ubuntu 24.04, pw 12345 |
| **JUPITER** | `192.168.8.21:50053` | still blocked from remus |

**SSH (triton):** `user@192.168.8.23`, password <redacted>.
...
- `sshpass` installed on remus (via sudo + pw) for simpler non-interactive `sshpass -p <redacted> ssh user@192.168.8.108 ...` to romulus (tested OK; complements the pexpect usage).
