#!/usr/bin/env bash
# B+10 scheduler spike: MoE async copy-slot wait ON (canonical) vs OFF on Tier A reference.
#
# usage:
#   bash scripts/b6-gate-b10-scheduler-spike.sh
#
# env:
#   B6_B10_PRESET    default b6-3gpu-g-triton
...
PRESET="${B6_B10_PRESET:-b6-3gpu-g-triton}"
MODEL="${BENCH_MODEL:-/mnt/models/Qwen3.6-35B-A3B-APEX-I-Quality.gguf}"
GEN="${BENCH_GEN_TOKENS:-384}"
OUT_BASE="${ROMULUS_REPO}/benches/path-b-plus/b6-b10-scheduler-spike-${STAMP}"
...
run_arm "canonical-moe-async-on" 1
run_arm "no-async-copy" 0
echo "B10_SCHEDULER_SPIKE_DONE dir=${OUT_BASE}"