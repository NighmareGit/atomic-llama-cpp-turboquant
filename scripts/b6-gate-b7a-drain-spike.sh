#!/usr/bin/env bash
# B+7a' 4-GPU drain spike: multi-socket RPC flush ON (canonical) vs OFF on b6-4gpu-g-triton.
#
# usage:
#   bash scripts/b6-gate-b7a-drain-spike.sh
#
# env:
#   BENCH_MODEL      default Qwen3.6-35B-A3B (4-GPU gate model)
...
OUT_BASE="${ROMULUS_REPO}/benches/path-b-plus/b6-b7a-drain-spike-${STAMP}"
...
run_arm() {
    ...
    echo "=== B+7a arm ${tag} MULTI_SOCKET_FLUSH=${flush} ==="
    set +e
    ssh_romulus "cd ${ROMULUS_REPO} && \\
      BENCH_MODEL='${MODEL}' BENCH_GEN_TOKENS=${GEN} \\
      GGML_PIPELINE_PLUS=1 \\
      GGML_RPC_MULTI_SOCKET_FLUSH=${flush} \\
      GGML_RPC_DUAL_SOCKET=0 B6_PERF_AUTO=0 \\
...
print('arm', row['arm'], 'G', row['G_tps'], 'ov', row['overlap_pct'], 'blocking', row['blocking_ms'], 'drain', row['drain_flush_ms'])
...
run_arm "canonical-flush-on" 1
run_arm "no-flush" 0
echo "B7A_DRAIN_SPIKE_DONE dir=${OUT_BASE}"