#!/usr/bin/env bash
# B+8 scheduler spike: partial pipeline_barrier ON vs OFF on frozen Tier A reference.
#
# usage:
#   bash scripts/b6-gate-b8-scheduler-spike.sh
#
# env:
#   B6_B8_PRESET     default b6-3gpu-g-triton
...
PRESET="${B6_B8_PRESET:-b6-3gpu-g-triton}"
MODEL="${BENCH_MODEL:-/mnt/models/Qwen3.6-35B-A3B-APEX-I-Quality.gguf}"
GEN="${BENCH_GEN_TOKENS:-384}"
OUT_BASE="${ROMULUS_REPO}/benches/path-b-plus/b6-b8-scheduler-spike-${STAMP}"
...
run_arm() {
    ...
    echo "=== B+8 arm ${tag} partial=${partial} strict=${strict} ==="
    set +e
    ssh_romulus "cd ${ROMULUS_REPO} && \\
      BENCH_MODEL='${MODEL}' BENCH_GEN_TOKENS=${GEN} \\
      GGML_PIPELINE_PLUS=1 \\
      GGML_PIPELINE_BARRIER_PARTIAL=${partial} \\
      GGML_PIPELINE_BARRIER_PARTIAL_STRICT=${strict} \\
...
print('arm', row['arm'], 'G', row['G_tps'], 'ov', row['overlap_pct'], 'serial', row['serial_dispatch_pct'])
...
run_arm "canonical-partial-strict" 1 1
run_arm "no-partial" 0 0
echo "B8_SCHEDULER_SPIKE_DONE dir=${OUT_BASE}"