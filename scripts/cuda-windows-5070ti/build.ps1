# Portable Windows CUDA build for Path B (llama-server + rpc-server).
# Collateral: scripts/cuda-windows-5070ti/  Docs: docs/cuda-windows-5070ti/
# Uses same build dir as rpc-patch Linux benches: build-cuda-b-bin/
param(
    [switch]$Clean,
    [switch]$SkipBuild,
    [switch]$Reconfigure,
...
Write-Host "=== cuda-windows-5070ti build ==="
...
if (-not $SkipBuild) {
    ...
    Write-Host "=== Build llama-server rpc-server (Release, -j $jobs) ==="
    Invoke-CmdBuild -VcVars $vcvars -Command "cmake --build build-cuda-b-bin --config Release -j $jobs -t llama-server rpc-server"
}
