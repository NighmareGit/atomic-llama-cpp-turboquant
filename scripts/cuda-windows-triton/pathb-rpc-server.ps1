# Triton RTX 3090 as Path B RPC worker on :50054 (B+6 remus-replacement spike).
param(
    [string]$Port = "50054",
...
function Resolve-RpcPortableDir {
    $candidates = @(
        ...
        "C:\projects\atomic-llama-cpp-turboquant\Path-B-Event-Support-Pipeline-Plus\build-cuda-b-bin\portable",
...
if (-not $PortableDir) {
    throw "rpc-server.exe not found - run scripts\cuda-windows\build.ps1 -Profile triton first"
}
...
Write-Host "B+6 spike endpoint: ${ip}:$Port"