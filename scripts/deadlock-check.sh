...
ssh -o ConnectTimeout=5 "hunter@192.168.8.22" "docker restart pathd-rpc-remus" > /dev/null 2>&1 || { echo "WARN: remus restart failed"; }
sleep 3
...
ssh -o ConnectTimeout=5 "hunter@192.168.8.22" "docker ps --filter name=pathd-rpc-remus --format '{{.Names}} {{.Status}}'" 2>/dev/null || { echo "ERROR: pathd-rpc-remus not found on remus"; exit 2; }
...
DOCKER_REMUS=$(ssh -o ConnectTimeout=5 "hunter@192.168.8.22" "docker logs pathd-rpc-remus" 2>&1 || echo "(ssh failed)")
