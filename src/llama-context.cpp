        ...
        if (st[stage] == -1) {
            ...
            for (auto & [seq_id, pos] : ctx->gpipe.seq_stage) {
                if (pos == stage && !used_this_cycle[seq_id]) {
                    ...
                    // D6.10: gate stage N completion via GPU event wait.
                    // CPU backend graph_compute is blocking (ggml_graph_compute
                    // waits for threadpool), so CPU is already drained here.
                    // GPU backends use ordered command streams — event wait on
                    // the GPU host backend gates all GPU backends safely.
                    if (stage > 0) {
...
llama_context::llama_context(
    ...
    cparams.override_moe_top_k      = params.override_moe_top_k;
    if (cparams.override_moe_top_k < 0) {
        throw std::runtime_error("override_moe_top_k must be >= 0");
    }
    if (cparams.override_moe_top_k > 0) {
        if (hparams.n_expert == 0) {
            LLAMA_LOG_WARN("%s: override_moe_top_k = %d ignored: model has no MoE experts\n",
                    __func__, cparams.override_moe_top_k);
        } else if ((uint32_t) cparams.override_moe_top_k > hparams.n_expert) {
            LLAMA_LOG_WARN("%s: override_moe_top_k = %d clamped to n_expert = %u\n",
                    __func__, cparams.override_moe_top_k, hparams.n_expert);
        } else {
            LLAMA_LOG_INFO("%s: override_moe_top_k = %d (native n_expert_used = %u)\n",
                    __func__, cparams.override_moe_top_k, hparams.n_expert_used);
        }
    }
