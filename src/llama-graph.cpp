...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    // [EXPERIMENTAL] --override-moe-top-k N: force the number of routed experts executed
    // (MoE top-k) to N instead of the model default. Clamped to [1, n_expert].
    // 0 = native behavior exactly (default).
    if (cparams.override_moe_top_k > 0) {
        n_expert_used = std::min<int64_t>(cparams.override_moe_top_k, n_expert);
    }
    ...
    // number of expert outputs to aggregate: the native count, or the effective
    // (overridden) count when --override-moe-top-k is active. the experts tensor
    // only has n_expert_used slices in the override case, so the native bound
    // would be out of bounds there.
    const uint32_t n_expert_used_aggr = cparams.override_moe_top_k > 0 ? (uint32_t) n_expert_used : hparams.n_expert_used;

    // order the views before the adds
    for (uint32_t i = 0; i < n_expert_used_aggr; ++i) {
    ...
    for (uint32_t i = 1; i < n_expert_used_aggr; ++i) {
        moe_out = ggml_add(ctx0, moe_out, cur_experts[i]);

        ggml_build_forward_expand(gf, moe_out);
    }

    if (n_expert_used_aggr == 1) {
