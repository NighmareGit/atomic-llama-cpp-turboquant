...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        ...
        // MTP head layers are tiny (~76 MiB total), so pin them to the first GPU
        // device regardless of the proportional split. Placing them on a remote
        // device forces cross-device hidden-state transfers that garble draft
        // predictions in speculative decoding.
...
int32_t llama_model_n_layer_all(const llama_model * model) {
