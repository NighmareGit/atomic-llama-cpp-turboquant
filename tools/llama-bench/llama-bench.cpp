...
struct cmd_params {
    ...
    std::vector<int>                 override_moe_top_k;
...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  --override-moe-top-k <n>                     (default: %s)\n", join(cmd_params_defaults.override_moe_top_k, ",").c_str());
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            ...
            } else if (arg == "--override-moe-top-k") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
                }
                auto p = parse_int_range(argv[i]);
                if (std::any_of(p.begin(), p.end(), [](int v) { return v < 0; })) {
                    invalid_param = true;
                    break;
                }
                params.override_moe_top_k.insert(params.override_moe_top_k.end(), p.begin(), p.end());
    ...
    if (params.override_moe_top_k.empty()) {
        params.override_moe_top_k = cmd_params_defaults.override_moe_top_k;
    }
...
struct cmd_params_instance {
    ...
    llama_context_params to_llama_cparams() const {
        ...
        cparams.override_moe_top_k = override_moe_top_k;
...
static std::vector<cmd_params_instance> get_cmd_params_instances(const cmd_params & params) {
    ...
    for (const auto & omtk : params.override_moe_top_k)
